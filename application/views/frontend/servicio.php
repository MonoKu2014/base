<div class="container">
  ...
  <div class="col-md-12 row">
    <div class="row">
          ...
          <div class="carousel-inner">
            ...
            foreach($imagenes as $k => $i){
                ...
                <div class="item <?= $class;?>">
                    <img src="<?= SERVICIOS_EMPRESA_PATH.$i->nombre_imagen;?>" alt="Producto <?= $k;?>">
                </div>
            <?php } } ?>
          </div>
        </div>
                ...
                <div class="item active">
                  ...
                  foreach($imagenes as $k => $i){ ?>
                      <div data-target="#carousel" data-slide-to="<?= $k;?>" class="thumb">
                        <img src="<?= SERVICIOS_EMPRESA_PATH.$i->nombre_imagen;?>" class="img-responsive" alt="Producto <?= $k;?>">
                      </div>
                  <?php if($k == 3){break;} } ?>
                </div>
      ...
      <div class="col-sm-6">
        <h5><?= $empresa[0]->nombre_empresa;?></h5><br class="visible-xs">
        <p><small><?= $this->functions->SCNombresPorEmpresa($empresa[0]->id_empresa);?></small></p>
        <h1 class="orange"><?= $servicio[0]->nombre_servicio;?></h1>

        <?php $descuento = $this->functions->precioDescuentoServicio($servicio[0]->id_servicio); ?>
        <?php $tipo_promocion = $this->functions->tipoPromocionServicio($servicio[0]->id_servicio); ?>

        <h3 class="precio">
          <?php if($descuento != '' && $tipo_promocion == 1){ ?>
            <small>$<?= $this->functions->moneda($servicio[0]->precio_servicio);?></small>
            <strong>$<?= $descuento;?></strong>
          <?php } else { ?>
            <strong>$<?= $this->functions->moneda($servicio[0]->precio_servicio);?></strong>
          <?php } ?>
        </h3>
        ...
        <div class="clearfix"></div>
        <?php if($this->functions->yaFueRecomendada($servicio[0]->id_empresa) == 1){?>
...
<br>
      ...
      <div class="col-md-6 col-sm-12">
        <h2 class="orange">Descripción del Servicio</h2>
        <p class="text-justify"><?= $servicio[0]->descripcion_servicio;?></p>
      </div>
      <div class="col-md-6 col-sm-12">
        <h2 class="orange">Especificaciones</h2>
        <p class="text-justify"><?= $servicio[0]->especificacion_servicio;?></p>
      </div>
      <div class="clearfix"></div>
      </div>
      ...
      <div class="col-sm-12 box-separator">
        <h2 class="orange">Otros Servicios de <?= $this->functions->nombreEmpresa($id_empresa);?></h2>
      ...
      <div class="col-sm-12 box-separator">
        <h2 class="orange">Productos ofrecidos por <?= $empresa[0]->nombre_empresa;?></h2>
        <div class="divisor"></div>
        <div class="row">
          ...
          <div id="recomendaciones-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="clearfix"></div>
                    <div class="col-lg-12">
                      <?php foreach($this->functions->listarRecomendacionesPorEmpresa($servicio[0]->id_empresa) as $k => $r){ ?>
                      <p><?= $k + 1;?>.-
    ...
    <div id="contactar-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h2 class="modal-title">Contactar por <b class="orange"><?= $servicio[0]->nombre_servicio;?></b></h2>
                <p>Tus datos vienen cargados de acuerdo a nuestra base de datos</p>
            </div>
            <div class="modal-body" id="modal-body" data-id="<?= $servicio[0]->id_empresa;?>" data-cliente="<?= $empresa[0]->id_cliente;?>">
            <div class="clearfix"></div>
              <form id="contactar-formulario">
                  <div class="col-md-12">
                    <div id="aviso-proceso"></div>
                      <h5>Servicio</h5>
                      <p></p>
                      <input type="text" class="form-control" required id="servicio"  value="<?= $servicio[0]->nombre_servicio;?>" />
                  </div>
...
$(document).ready(function(){
  ...
  $('#envia-contacto').on('click', function(event){
        ...
        if(nombre.trim() == '' || email.trim() == '' || mensaje.trim() == ''){
              ...
              $.ajax({
                  type: 'post',
                  url: APP_URL + 'ajax/contactar_producto',
                  data: {id_empresa:id_empresa, servicio:servicio, nombre:nombre, email:email, mensaje:mensaje, id_cliente},
                  dataType: 'json',
                  success: function(res){
                      $('#aviso-proceso').html(res);
                      if(res.estado == 0){
  ...
  $(document).on('click', '#boton-quitar-recomendacion', function(){
      ...
      if(session == ''){
          ...
          $.ajax({
              ...
              data: {id_empresa:id_empresa, id_cliente},
              success: function(res){
